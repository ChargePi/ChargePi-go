...
type CS5460Settings struct {
	SPI                  *hardware.SPI `json:"spi,omitempty" yaml:"spi,omitempty" mapstructure:"spi,omitempty"`
...
func (settings *CS5460Settings) Validate() error {
	if settings.SPI == nil {
		return errors.New("spi settings are missing")
	}

	if settings.SPI.ChipSelect <= 0 {
		return errors.New("chip select pin cannot be negative")
	}
...
func NewCS5460PowerMeter(logger *zap.Logger, settings CS5460Settings) (*C5460A, error) {
	if settings.SPI.ChipSelect <= 0 {
		return nil, errors.New("pin cannot be negative")
	}

	if settings.SPI.Bus < 0 {
		return nil, errors.New("spi bus cannot be negative")
	}

	return &C5460A{
		logger:               logger.Named("cs5460a"),
		pin:                  settings.SPI.ChipSelect,
		bus:                  settings.SPI.Bus,
		voltageDividerOffset: settings.VoltageDividerOffset,
		ShuntOffset:          settings.ShuntOffset,
		voltageMultiplier:    VoltageRange * settings.VoltageDividerOffset,
		currentMultiplier:    CurrentRange * settings.ShuntOffset,
		powerMultiplier:      settings.ShuntOffset * settings.VoltageDividerOffset,
	}, nil
}
...
func (receiver *C5460A) Cleanup() error {
